...
<User
    ...
    description={{ "silhouette": "mince", "dateAnniversaire": "01/01/1970", "poids": 80 }}
...
Si le nom de la props est le même que le nom de la variable à passer dans le composant, vous pouvez écrire comme ceci :
...
Dans l'exemple précédent, la logique est appliquée à l'instruction réactive.
...
Vous pouvez utiliser le `Sass`. Cependant, il va falloir configurer `rollup.config.js` ou `webpack.config.js`. Une fois configuré, vous n'avez plus qu'à déclarer votre élément style comme ceci :
...
Nous allons finir ce cours par créer un composant relativement classique. Si vous avez lu le cours, vous ne devriez pas être trop perdu. Nous verrons dans d'autres cours, les notions de passage de variables via les événements ou les structures de contrôle.
